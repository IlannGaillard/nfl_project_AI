...
rf_classifier = RandomForestClassifier(
    n_estimators=100,       # On garde 200 arbres pour une bonne stabilité
...
st.title("🏈 Prédiction de match NFL (2025 et au-delà)")
st.write(f"📊 Précision du modèle : {accuracy * 100:.2f}%")

team1_input = st.text_input("Entrez le nom de l'équipe à domicile :", "")
team2_input = st.text_input("Entrez le nom de l'équipe à l'extérieur :", "")

if st.button("🔮 Prédire le match"):
    team1 = team1_input.strip()
    team2 = team2_input.strip()

    # Normalisation des noms d'équipes avec la fonction normalize_team_name
    team1 = normalize_team_name(team1)
    team2 = normalize_team_name(team2)

    # Vérification que les équipes normalisées sont valides
    available_teams = df["home_team"].unique()

    if team1 not in available_teams or team2 not in available_teams:
        st.error(f"❌ L'une des équipes n'est pas valide. Équipes valides : {list(available_teams)}")
    else:
        ...
        if prob_home_win is None:
            ...
            # Convertir les abréviations en noms complets
            team1_full_name = TEAM_FULL_NAMES.get(team1, team1)  # Si l'abréviation n'est pas trouvée, garder l'abréviation
            team2_full_name = TEAM_FULL_NAMES.get(team2, team2)

            # Affichage des résultats avec les noms complets
            st.write(f"📊 **{team1_full_name}** a **{prob_home_win * 100:.2f}%** de chances de gagner.")
            st.write(f"📊 **{team2_full_name}** a **{(1 - prob_home_win) * 100:.2f}%** de chances de gagner.")
